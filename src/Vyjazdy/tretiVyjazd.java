...
public class tretiVyjazd extends Vyjazd {
		...
		super("Treti Vyjazd", new HashMap<String, PIDController>(){
			{
				...
				put("dozaduLavy", new PIDController((0.04/400)*PD,(40/400)*nieco,(50/400)*PD));// (0.2/400)*PD,(100/400)*nieco,(250/400)*PD)                6.8  110  500               //1,0.6,2.5         // 3.2,2 //1.2,0.7,2.0     //0.75,0.6,0.4 - toto
				put("dozaduPravy", new PIDController((-0.04/400)*PD,(-40/400)*nieco,(-50/400)*PD));//-1,-0.6,-2.5// d samostatne 1.4 //0.1,0,0.65
			}
		});
	}
	...
	protected void Spusti() throws InterruptedException {		
		...
		robot.dopreduBezGyra2(1.44-0.015, 700, 1000, 870);//robot.dopredubezgyra(1.438, 700);//1.443
		float uhol1 = 87;
		robot.otocitPoUhol(120, 600, uhol1);//100,500
		robot.dopredu(0.2775 + 0.10,600,uhol1);//0.2825
		TimeUnit.SECONDS.sleep(3);
		robot.otocitPoUhol(100, -100, 85);
		robot.otocitPoUhol(-100, 100, 90);
		robot.dozadu(0.10, 600);
		...
		robot.otocitPoUhol(200, -400, 90-180);
		robot.dozadu(0.55,600);
		robot.dopredu(0.26,600);
		robot.otocitPoUhol(-200, 100, uhol2-5-180);
